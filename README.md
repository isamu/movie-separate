...
4. **フェーズ1: 各セグメントの文字起こしと翻訳**:
   - **キャッシュチェック**: セグメントのすべてのファイル（動画、音声、サムネイル）が存在し、キャッシュがある場合は**すべてスキップ**（Whisper、翻訳、話者識別のAPI呼び出しなし）
   - 動画ファイル（1.mp4, 2.mp4, ...）を生成
   - サムネイル画像（1.jpg, 2.jpg, ...）を生成
   - 音声ファイル（1.mp3, 2.mp3, ...）を抽出
   - Whisper APIで日本語文字起こし
   - GPT-4o-miniで英語に翻訳（翻訳キャッシュにある場合はスキップ）
   - GPT-4oで話者を識別
   - 各セグメント処理後にJSONを保存（クラッシュ時の安全性）
5. **フェーズ2: 日本語TTS音声生成**:
   - 日本語音声ファイル（1_ja.mp3, 2_ja.mp3, ...）を生成
   - 既存ファイルがある場合はスキップ
...
このツールは3段階のキャッシュシステムでAPIコストを最小化します：

### 1. セグメント全体のキャッシュ（最も効率的）

- すべてのファイル（動画、音声、サムネイル）が存在し、`mulmo_view.json`にデータがある場合
- **スキップされる処理**: Whisper API、翻訳API、話者識別API、ffmpeg処理
- **コスト削減**: セグメントあたり約$0.10〜$0.30

### 2. 翻訳キャッシュ

- 同じ日本語テキストが既に翻訳されている場合、再翻訳をスキップ
- 異なるセグメントで同じ内容が話された場合に有効
- **コスト削減**: 翻訳ごとに約$0.01〜$0.03

### 3. TTS音声ファイルキャッシュ

- `{n}_ja.mp3`ファイルが既に存在する場合、TTS生成をスキップ
- フェーズ2で独立してチェック
- **コスト削減**: セグメントあたり約$0.01〜$0.05

### 再実行時の動作

処理が中断された場合や、既存データがある場合：

```bash
npm start  # 同じ動画を再実行
```

- 既に完了したセグメント → すべてスキップ（コスト: $0）
- 未完了のセグメント → 通常通り処理
- 既存の翻訳 → 再利用
- 既存のTTS音声 → 再利用

**重要**: ファイルを手動で削除した場合は、そのセグメントは再処理されます。
